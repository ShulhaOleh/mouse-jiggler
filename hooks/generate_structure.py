...
def run_generator():
    bat = SCRIPTS / "generate_structure.bat"
    ps1 = SCRIPTS / "generate_structure.ps1"

    # Maybe in the future for linux
    # sh = SCRIPTS / "generate_structure.sh"

    if os.name == "nt" and bat.exists():
        subprocess.check_call([str(bat)], shell=True)
        return
    if os.name == "nt" and ps1.exists():
        subprocess.check_call(["powershell", "-NoProfile", "-NonInteractive", "-ExecutionPolicy", "Bypass", "-File", str(ps1)])
        return

    raise FileNotFoundError("No generator script found in scripts/ (generate_structure.bat/.ps1)")
